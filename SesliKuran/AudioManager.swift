...
class AudioManager: ObservableObject {
    ...
    func downloadTrack(surah: Surah) {
        ...
        // Using a reliable public source. MP3Quran (Mishary Alafasy)
        // Format: https://server8.mp3quran.net/afs/001.mp3
        let formattedId = String(format: "%03d", surah.id)
        let urlString = "https://server8.mp3quran.net/afs/\(formattedId).mp3"

        guard let url = URL(string: urlString) else {
            errorMessage = "Ungültige Download-URL"
            isDownloading = false
            return
        }
        ...
        downloadTask = URLSession.shared.downloadTask(with: url) { [weak self] tempURL, response, error in
            ...
            let destinationURL = documentDirectory.appendingPathComponent("Audio \(surah.id).mp3")
